...
public final class TripStore {
    ...
    public static synchronized JSONObject save(Context context,JSONObject trip)throws Exception{
        JSONArray trips=all(context);String id=trip.optString("","");id=trip.optString("id","");
        if(id.isEmpty()){id=UUID.randomUUID().toString();trip.put("id",id);}if(!trip.has("status"))trip.put("status","queued");
        boolean replaced=false;for(int i=0;i<trips.length();i++){if(id.equals(trips.getJSONObject(i).optString("id"))){trips.put(i,trip);replaced=true;break;}}
        if(!replaced)trips.put(trip);write(context,trips);return trip;
    }
    ...
    public static synchronized void updateStatus(Context context,String id,String status){
        try{JSONObject t=get(context,id);if(t!=null){t.put("status",status);save(context,t);HistoryStore.syncStatus(context,t,status);}}catch(Exception ignored){}
    }
