...
public final class TripStore {
    ...
    public static synchronized JSONArray all(Context context) {
        try {
            return new JSONArray(context.getSharedPreferences(PREFS, 0).getString(KEY_TRIPS, "[]"));
        } catch (Exception e) {
    ...
    public static synchronized JSONObject save(Context context, JSONObject trip) throws Exception {
        JSONArray trips = all(context);
        String id = trip.optString("id", "");
        if (id.isEmpty()) {
            id = UUID.randomUUID().toString();
            trip.put("id", id);
        }
        if (!trip.has("status")) trip.put("status", "queued");
        boolean replaced = false;
        for (int i = 0; i < trips.length(); i++) {
            if (id.equals(trips.getJSONObject(i).optString("id"))) {
                trips.put(i, trip);
                replaced = true;
                break;
            }
        }
        if (!replaced) trips.put(trip);
        write(context, trips);
        return trip;
    }
    ...
    public static synchronized boolean delete(Context context, String id) {
        JSONArray src = all(context);
        JSONArray dst = new JSONArray();
        boolean removed = false;
        for (int i = 0; i < src.length(); i++) {
            JSONObject t = src.optJSONObject(i);
            if (t != null && id.equals(t.optString("id"))) removed = true;
            else dst.put(t);
        }
        write(context, dst);
        return removed;
    ...
    private static void write(Context context, JSONArray trips) {
        context.getSharedPreferences(PREFS, 0).edit().putString(KEY_TRIPS, trips.toString()).apply();
    }
}