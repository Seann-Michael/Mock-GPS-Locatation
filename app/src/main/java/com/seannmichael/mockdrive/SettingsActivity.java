...
public class SettingsActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        button(root, "Generate New API Key").setOnClickListener(v -> {
            String key = AppPreferences.generateApiKey(this);
            refreshApi();
            copy(key);
            toast("New API key generated and copied. The previous key is no longer valid.");
        });
        button(root, "Copy Current API Key").setOnClickListener(v -> {
            String key = AppPreferences.apiKey(this);
            if (key == null || key.startsWith("revoked-")) toast("No active API key"); else { copy(key); toast("API key copied"); }
        });
        button(root, "Revoke API Key").setOnClickListener(v -> {
            AppPreferences.revokeApiKey(this);
            refreshApi();
        ...
        placesKey = field(root, "Google Places API key", GoogleBusinessResolver.apiKey(this));
        placesKey.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        button(root, "Save Google Places Key").setOnClickListener(v -> {
            GoogleBusinessResolver.setApiKey(this, placesKey.getText().toString().trim());
            toast("Google Places key saved locally");
        });
        ...
        button(root, "Save Preferences").setOnClickListener(v -> {
            AppPreferences.setUnits(this, units.getSelectedItem().toString());
            AppPreferences.setTheme(this, theme.getSelectedItem().toString());
    ...
    private void refreshApi() {
        String key = AppPreferences.apiKey(this);
        boolean active = key != null && !key.startsWith("revoked-");
        apiStatus.setText(active ? "Active key: " + key.substring(0, Math.min(14, key.length())) + "…" : "No active API key");
    }
