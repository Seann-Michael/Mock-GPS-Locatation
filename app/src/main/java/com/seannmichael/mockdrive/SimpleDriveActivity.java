...
public class SimpleDriveActivity extends BaseActivity {
    ...
    @Override protected void onCreate(Bundle state){
        ...
        speedBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){
            @Override public void onProgressChanged(SeekBar seekBar,int progress,boolean fromUser){
                selectedSpeed=progress+5;
                speedValue.setText(selectedSpeed+" mph");
            }
    ...
    private void setupAutocomplete(AutoCompleteTextView input,EditText lat,EditText lon,String label){
        ...
        input.setOnItemClickListener((parent,view,position,id)->{
            ...
            status.setText("Loading "+label.toLowerCase()+" details…");
            new Thread(()->{
                try{
                    JSONObject place=GooglePlacesEngine.placeDetails(this,placeId);
                    runOnUiThread(()->{
                        input.setText(place.optString("formattedAddress",place.optString("label","")),false);
                        lat.setText(String.valueOf(place.optDouble("latitude")));
                        lon.setText(String.valueOf(place.optDouble("longitude")));
                        input.dismissDropDown();
                        status.setText(label+" selected: "+place.optString("label"));
                    });
                }catch(Exception e){
                    runOnUiThread(()->status.setText("Could not load place: "+friendlyPlacesError(e)));
                }
            },"place-details").start();
        });
        input.addTextChangedListener(new TextWatcher(){
            ...
            @Override public void onTextChanged(CharSequence s,int st,int before,int count){
                ...
                if(q.length()<2){
                    labels.clear();ids.clear();adapter.notifyDataSetChanged();input.dismissDropDown();return;
                }
                pending=()->new Thread(()->{
                    try{
                        JSONArray results=GooglePlacesEngine.autocomplete(SimpleDriveActivity.this,q);
                        ArrayList<String> nextLabels=new ArrayList<>();
                        ArrayList<String> nextIds=new ArrayList<>();
                        for(int i=0;i<results.length();i++){
                            JSONObject r=results.optJSONObject(i);
                            if(r!=null){nextLabels.add(r.optString("label"));nextIds.add(r.optString("placeId"));}
                        }
                        runOnUiThread(()->{
                            if(!q.equals(input.getText().toString().trim()))return;
                            labels.clear();labels.addAll(nextLabels);
                        ...
                        runOnUiThread(()->status.setText(friendlyPlacesError(e)));
                    }
    ...
    private String friendlyPlacesError(Exception e){
        ...
        if(message.contains("403"))return "Google Places denied the request. Check Places API (New), billing, and key restrictions.";
        return message;
    }
            ...
            new Handler().postDelayed(()->{
                try{
                    String url="https://www.google.com/maps/dir/?api=1&origin="+aLat+","+aLon+
                            "&destination="+Uri.encode(bLat+","+bLon)+"&travelmode=driving&dir_action=navigate";
                    ...
                    status.setText("Navigation active at "+selectedSpeed+" mph.");
                }catch(Exception e){
                    status.setText("Could not open Google Maps: "+e.getMessage());
                }
            },1200);
        }catch(Exception e){
